...
struct Event create_new_event(int hour, int min, int sec, int event_id) {
  struct Time event_time = { .hour = hour, .min = min, .sec = sec };
  struct Event ret = { .time = event_time, .event_id = event_id };
  return ret;
}
...
struct Time get_event_time(struct Event e) {
  struct Time ret = {.hour = e.time.hour, .min = e.time.min, .sec = e.time.sec};
  return ret;
}
...
struct Time elapsed_time(struct Event e1, struct Event e2) {
  ...
  struct Time e1_time = get_event_time(e1);
  struct Time e2_time = get_event_time(e2);
  int e1_sec = totalFromHMS(e1_time.hour, e1_time.min, e1_time.sec);
  int e2_sec = totalFromHMS(e2_time.hour, e2_time.min, e2_time.sec);
  int diff = e1_sec - e2_sec;
  if (diff < 0){
      diff *= -1;
  }
  int hour = totalToHours(diff);
  diff -= hour* 3600;
  int min = totalToMinutes(diff);
  diff -= min * 60;
  int sec = totalToSeconds(diff);
  struct Time ret = {.hour = hour, .min = min, .sec = sec};
  return ret;
